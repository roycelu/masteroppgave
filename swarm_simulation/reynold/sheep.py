...
class Sheep:
    def __init__(self, initial_position, id):
        ...
        self.cohesion_weight = 20
        self.separation_weight = 20
        self.separation_weight_drones = 40
    ...
    def main_sheep(self, list_of_sheep, canvas, list_of_drones):
        ...
        self.acceleration += v1 + v2 + v3 + v2_2
    ...
    def cohesion(self, nearest_sheep):
        ...
        if total > 0:
            ...
            # Normalize the vector towards the center of mass, because we want direction
            if np.linalg.norm(vec_to_com) > 0:
                vec_to_com = (vec_to_com / np.linalg.norm(vec_to_com)) * self.max_speed
            steering = vec_to_com - self.velocity
            # Normalize the vector for steering, because we want direction
            if np.linalg.norm(steering) > self.max_force:
                steering = (steering /np.linalg.norm(steering)) * self.cohesion_weight
    ...
    def separation(self, nearest_sheep):
        ...
        # Keep track of distances and divide the diff vector (the direction of escape) by the distance to that particular flockmate
        for boid in nearest_sheep:
            ...
            if ((boid.position[0] != self.position[0]) and (boid.position[1] != self.position[1])) and distance < self.horizon:
        ...
        if total > 0:
            ...
            if np.linalg.norm(steering) > self.max_force:
                steering = (steering /np.linalg.norm(steering)) * self.separation_weight
    ...
    def drone_separation(self, nearest_drone):
        ...
        if total > 0:
            ...
            steering = avg_vector - self.velocity
            if np.linalg.norm(steering) > self.max_force:
                steering = (steering /np.linalg.norm(steering)) * self.separation_weight_drones
    ...
    def alignment(self, nearest_sheep):
        ...
        if N > 0:
            ...
            avg_vector = (avg_vector / np.linalg.norm(avg_vector)) * self.alignment_weight
            steering = avg_vector - self.velocity
