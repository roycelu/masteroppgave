...
def test(id, no_sheep, no_drones, FPS, dronetype, testtype, initial_goal_vector, initial_goal):
    ...
    successrate, herdtime, reached_goal_times, reached_goal_number = sim.main()
...
def main():    
    ...
    df_circle_original.to_csv('circle.csv', index=False)

    # df_occlusion_original = df_occlusion_original.drop(columns=['Dronetype'])
    # df_occlusion_original.to_csv('occlusion.csv', index=False)

    # df_polygon_original.drop(columns=['Dronetype'])
    # df_polygon_original.to_csv('polygon.csv', index=False)
    ...
    # df_polygon = df_occlusion.groupby(['Testtype', 'Dronetype'], as_index=False).aggregate({'Suksess': 'sum', 'Failure':'sum'})
