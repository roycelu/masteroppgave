...
def test(id, no_sheep, no_drones, FPS, dronetype, testtype):
    ...
    successrate, herdtime, reached_goal_times, reached_goal_number = sim.main(TIME_LIMIT, TARGET_FPS)
    ...
    for dronetype in DRONETYPES:
        for testtype in TESTTYPES:
            for id in range(NO_SIMULATIONS):
                ...
                successrate, herdtime, reached_goal_times, reached_goal_number = test(id, NO_SHEEP, NO_DRONES, FPS*NO_DRONES, dronetype, testtype)
                if dronetype == 'circle':
