...
class OurDroneFurthest:
    ...
    def allocate_steering_points(self, drones, sheep, com, goal):
        ...
        steering_points = []
        for m in range(len(sheep_list)):
            for n in range(len(sheep_list) - m - 1):
                d = com.distance_to(sheep_list[n])
                d2 = com.distance_to(sheep_list[n + 1])
                if d > d2:
                    sheep_list[n], sheep_list[n + 1] = sheep_list[n + 1], sheep_list[n]
        steering_points = sheep_list[:len(drones):-1]
